...
@section('content')
    <div class="w-full max-w-2xl mx-auto bg-gray-800 rounded-3xl shadow-2xl p-10 border border-gray-700">
        <h2 class="text-3xl font-extrabold text-center text-white mb-10">Editar Lead</h2>
        ...
        <form method="POST" action="{{ route('leads.update', $lead->id) }}" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4" required
                          class="w-full bg-gray-900 border-2 border-gray-600 text-white focus:border-orange-500 focus:ring-orange-500 rounded-xl px-5 py-3 shadow-sm placeholder-gray-500">{{ old('description', $lead->description) }}</textarea>
                @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="estimated_value" value="{{ old('estimated_value', $lead->estimated_value) }}" required
                       class="w-full bg-gray-900 border-2 border-gray-600 text-white focus:border-orange-500 focus:ring-orange-500 rounded-xl px-5 py-3 shadow-sm placeholder-gray-500">
            ...
            <div>
                ...
                <select name="interest_levels" required
                    ...
                    <option value="Frio" {{ (old('interest_levels') ?? $lead->interest_levels) == 'Frio' ? 'selected' : '' }}>Frio</option>
                    <option value="Morno" {{ (old('interest_levels') ?? $lead->interest_levels) == 'Morno' ? 'selected' : '' }}>Morno</option>
                    <option value="Quente" {{ (old('interest_levels') ?? $lead->interest_levels) == 'Quente' ? 'selected' : '' }}>Quente</option>
                </select>
                ...
                <div>
                    ...
                    <select name="owner_id" required
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ (old('owner_id') ?? $lead->owner_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                    @error('owner_id')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
